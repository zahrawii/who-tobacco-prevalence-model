...
#
#   Generates publication-quality GT tables for manuscript and supplementary materials.
#
...
#     - Uses format_gender() from 00_config.R for "Men"/"Women" labels
#     - Each supplementary row = gender x country x indicator stratum
#
#   Requires: 00_config.R (constants, format_gender, format_indicator, INDICATOR_LABELS)
#
#########################################################################################
...
# Uses constants from 00_config.R: BASE_YEAR, TARGET_YEAR, TARGET_YEAR_2030,
# ENDGAME_YEAR, REDUCTION_TARGET, ENDGAME_THRESHOLD_PROP, NEAR_ENDGAME_PROP,
# VIRTUAL_ELIMINATION_PROP, ON_TRACK_MARGIN, INDICATOR_LABELS, format_gender()
# ============================================================================

# Map config names to table-specific names used throughout
BASELINE_YEAR   <- BASE_YEAR        # 2010
COMPARISON_YEAR <- 2024             # Latest available data year
INTERIM_YEAR    <- TARGET_YEAR      # 2025 (WHO interim assessment)
# Note: Tables use TARGET_YEAR_2030 for the WHO 2030 target
# The global TARGET_YEAR (2025) is the interim target

# Primary indicator for main tables (WHO FCTC priority)
TABLES_TABLES_PRIMARY_INDICATOR <- "current_user_any_tobacco_product"

# Short labels for column headers (table-specific)
INDICATOR_SHORT <- c(
...
# (format_gender and format_indicator come from 00_config.R)
# ============================================================================
...
generate_table1 <- function(clean_data,
  ...
  prevalence_comparison <- weighted_results %>%
    filter(!!sym(indicator_col) == TABLES_PRIMARY_INDICATOR) %>%
  ...
  survey_characteristics <- clean_data %>%
    filter(def_type_code == TABLES_PRIMARY_INDICATOR) %>%
    left_join(
      country_region_mapping %>% select(wb_country_abv, region_consolidated),
      by = c("wb_country_abv")
    ) %>%
  ...
  gt_table1 <- table1_final %>%
    ...
    tab_style(
      style = list(
        cell_fill(color = TABLE_COLORS$row_group),
    ...
    tab_style(
      style = list(
        cell_fill(color = TABLE_COLORS$subtotal),
    ...
    tab_options(
      ...
      heading.title.font.size = px(14),
      heading.title.font.weight = "bold",
      column_labels.background.color = TABLE_COLORS$header,
      column_labels.font.weight = "bold",
      row_group.background.color = TABLE_COLORS$row_group,
      data_row.padding = px(6)
    )
...
generate_table2 <- function(weighted_results,
  ...
  predictions <- weighted_results %>%
    filter(!!sym(indicator_col) == TABLES_PRIMARY_INDICATOR,
           !!sym(year_col) %in% c(INTERIM_YEAR, TARGET_YEAR_2030)) %>%
    select(!!sym(country_col), !!sym(sex_col), !!sym(year_col), !!sym(prev_col)) %>%
    distinct() %>%
    rename(Country = !!sym(country_col), Sex = !!sym(sex_col),
           Year = !!sym(year_col), Prevalence = !!sym(prev_col)) %>%
  ...
  pred_target_col <- paste0("pred_", TARGET_YEAR_2030)
  ...
  gt_table2 <- table2_final %>%
    ...
    tab_style(
      style = cell_text(color = TABLE_COLORS$positive_change, weight = "bold"),
...
generate_table3 <- function(weighted_results,
  ...
  endgame_status <- predictions_2040 %>%
    ...
    filter(!is.na(region_consolidated)) %>%
    mutate(
      achieved_endgame = pred_2040 < ENDGAME_THRESHOLD_PROP,
      near_endgame = pred_2040 >= ENDGAME_THRESHOLD_PROP & pred_2040 < NEAR_ENDGAME_PROP,
      virtual_elimination = pred_2040 < VIRTUAL_ELIMINATION_PROP
    )
...
generate_table_s1 <- function(weighted_results,
  ...
  gt_table_s1 <- table_s1_wide %>%
    ...
    tab_style(
      style = list(
        cell_fill(color = TABLE_COLORS$header),
        cell_text(weight = "bold")
      ),
...
generate_table_s3 <- function(weighted_results,
  ...
  gt_table_s3 <- table_s3_data %>%
    ...
    tab_style(
      style = cell_fill(color = TABLE_COLORS$good),
      locations = cells_body(
        columns = Status,
    ...
    tab_style(
      style = cell_fill(color = TABLE_COLORS$low),
      locations = cells_body(
        columns = Status,
